...
// ==========================================
// 4. INIT (BAŞLATMA - SÜRƏTLİ VERSİYA)
// ==========================================

document.addEventListener('DOMContentLoaded', () => {
    // 1. DÜYMƏLƏRİ AKTİV ET
    ...
    // 2. YADDAŞDAN OXU (Gözləmək yoxdur!)
    const session = localStorage.getItem('activeUser');
    if (session) {
        try {
            currentUser = JSON.parse(session);
            checkSession(); // Menyuları (Giriş/Çıxış) tənzimlə
            updateUserUI(); // Yaddaşdakı köhnə balansı dərhal göstər
        } catch (e) {
            console.log("Sessiya xətası, sıfırlandı.");
            localStorage.removeItem('activeUser');
        }
    }

    // 3. EKRANI DƏRHAL QUR (Məhsulları göstər)
    createModalHTML();
    renderUserHome(); // İstifadəçi dərhal məhsulları görür (Donma yoxdur)

    // 4. LOADER-İ SÖNDÜR (Maksimum 0.5 saniyəyə ekran açılır)
    // Artıq API-ni gözləmirik, ekran açılır, istifadəçi gəzir
    setTimeout(() => {
        toggleLoading(false);
    }, 500);

    // ============================================================
    // 5. ARXA PLAN (BACKGROUND) PROSESLƏRİ
    // (Sayt işləyərkən altdan-altdan məlumatı təzələyir)
    // ============================================================

    // A) Məlumat Bazasını (Məhsulları) Yenilə (Public Data)
    ...
    if (currentUser && currentUser.username && currentUser.password) {
        ...
        sendRequest({
            ...
            if (res.status === "success") {
                // Serverdən gələn təzə məlumat
                const savedPass = currentUser.password; // Şifrəni qoruyuruq
                currentUser = res.user;
                currentUser.password = savedPass;

                // Yaddaşı və Ekranı Yenilə
                localStorage.setItem('activeUser', JSON.stringify(currentUser));
                updateUserUI(); // Balans yeniləndi (məsələn: 0.00 -> 12.50)
                console.log("Balans uğurla yeniləndi.");
            }
        }).catch(e => {
            console.log("Serverlə əlaqə yoxdur, köhnə balans ekranda qaldı.");
        });
    }
});
